```sequence
Title: QQ登录时序图
浏览器->服务器: 请求用于QQ登录的URL网址
服务器->浏览器: 返回QQ登录网址
浏览器-->QQ服务器: 用户在QQ网页进行登录
QQ服务器-->浏览器: 用户在QQ登录成功，QQ将用户重定向到服务器的callback网址，并携带授权code
浏览器->服务器: 访问callback?code=xxx网址
...
服务器-->浏览器: 如果用户不是第一次QQ登录，则登录成功，返回JWT token

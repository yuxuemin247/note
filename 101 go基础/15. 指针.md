...
![image-20190209154549871](image-20190209154549871.png)
...
我们向函数 `change` 传递了指针变量 `b`，而 `b` 存储了 `a` 的地址。在 `change` 函数内使用解引用，修改了 a 的值。该程序会输出：
...
**这种向函数传递一个数组指针参数，并在函数内修改数组。尽管它是有效的，但却不是 Go 语言惯用的实现方式。我们最好使用切片来处理。**

...
上面的代码创建了一个名为 m1 的 map，其中键是 string 类型，而值是 int 类型。
...
#这种方式构造的  m1 := make(map[string]int) 
# m1不是nil 
```

上面的程序中，m1 是 nil，因此需要使用 make 方法初始化，程序将输出 `map is nil. Going to make one.`。
...
上面的程序声明了 m1，并在声明的同时添加两个元素。之后又添加了键 `mike`。程序输出：

```
map[steve:12000 jamie:15000 mike:9000]
```

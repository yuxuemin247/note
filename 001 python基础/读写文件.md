...
​                    mode：文件打开模式
 		   ...
 		   encodeing: 文件编码方式，不用于二进制文件，一般是utf-8,gbk，在针对b字节数据处理时，不能指定encoding参数
...
| b    | rb、wb、ab、rb+、wb+、ab+意义和上面一样，用于二进制文件操作，一般是对媒体文件(图片、视频、音频、pickle序列化的对象数据) |
...
~~~
 ...
 'r+'和'w+'的区别是什么？ 都是可读可写，但r+模式下，如果文件不存在，则会异常，而w+则会创建文件。
...
```
with open('a.txt') as f:  # 20G，  100T
    # 文件流对象是可以被迭代的
    # 高性能的操作， 节省内存空间
    # f.seek() 可以从指定的位置上读取多个字符或字节的数据， 与read()组件使用
    for line in f:
         print(line)
```

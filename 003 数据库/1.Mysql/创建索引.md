- 在创建表时创建索引 使用 key  索引名 (字段名)

  ```
  DROP TABLE IF EXISTS `film`; CREATE TABLE `film` (   `id` int(11) NOT NULL AUTO_INCREMENT,   `name` varchar(10) DEFAULT NULL,   PRIMARY KEY (`id`),   KEY `idx_name` (`name`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
  ```
...
Cannot use ref access on index 'PRIMARY' due to type or collation conversion on field 'order_id' 
由于字段“order_id”上的类型或排序转换，无法在索引“主”上使用ref访问。
```
...
- `isnull` 和 `is not null `也无法使用索引

- like以通配符开头,`mysql`索引会失效导致全表扫描操作,用覆盖索引优化 like '%字符串%'

  - 少用or或者in,用他查询时,非主键字段的索引会失效,主键索引有事生效,有时不生效,跟数据量有关,具体还得看mysql得查询优化结果
...
在关系数据库中，索引是一种单独的、物理的对数据库表中一列或多列的值进行排序的一种存储结构; 也称之为`key`
...
- 建索引的几大原则
  - 最佳左前缀原则
  - 尽量选择区分度高的列作为索引，如果唯一就更好了。
  - 索引列不能参与计算 比如 from_unixtime(create_time) ='2019-08-02'就不能使用索引了，应该协程 create_time = unix_timestamp('2018-08-02')
